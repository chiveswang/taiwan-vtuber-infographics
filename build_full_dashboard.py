...
function buildConc(inclP){
  ...
  function renderOutliers(rec){
    ...
    const ps=periods(), idx=Math.max(0,+periodEl.value||0), win=ps.slice(Math.max(0,idx-2),idx+1);
    const flowById={};
    win.forEach(p=>(p.channels||[]).forEach(c=>{if(c.id&&c.pv!=null&&isFinite(c.pv))flowById[c.id]=(flowById[c.id]||0)+Math.max(0,c.pv);}));
    const rows=(rec.channels||[]).filter(c=>c.id&&c.ac==="Active"&&c.s>0).map(c=>Object.assign({},c,{rpv:flowById[c.id]||0}));
    const nonzero=rows.filter(c=>c.rpv>0);
    const subs=rows.map(c=>c.s).sort((a,b)=>a-b), flows=nonzero.map(c=>c.rpv).sort((a,b)=>a-b);
    const totalS=rows.reduce((a,c)=>a+c.s,0), totalPv=nonzero.reduce((a,c)=>a+c.rpv,0);
    const pts=nonzero.map(c=>{const sp=pctRank(subs,c.s), fp=pctRank(flows,c.rpv), fps=c.rpv/c.s, shareRatio=(totalS&&totalPv)?((c.rpv/totalPv)/(c.s/totalS)):null;return Object.assign({},c,{x:sp,y:fp,fps,shareRatio});}).filter(c=>c.x!=null&&c.y!=null);
    store.ccOutlier=new Chart(document.getElementById("ccOutlier"),{type:"scatter",data:{datasets:[{label:"頻道",data:pts,backgroundColor:pts.map(p=>p.x>=80&&p.y<=20?C.red+"cc":p.x<=50&&p.y>=90?C.green+"cc":p.shareRatio>=3?C.amber+"cc":C.blue+"66"),pointRadius:pts.map(p=>p.x>=80&&p.y<=20||p.x<=50&&p.y>=90||p.shareRatio>=3?6:3),pointHoverRadius:8}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>`${ctx.raw.n}｜訂閱 ${ctx.raw.x}% / 近3期期間觀看 ${ctx.raw.y}%｜近3期期間觀看/訂閱 ${ctx.raw.fps.toFixed(2)}`}}},scales:{x:{min:0,max:100,ticks:{color:TICK,callback:v=>v+"%"},grid:{color:GRID},title:{display:true,text:"訂閱數百分位",color:TICK}},y:{min:0,max:100,ticks:{color:TICK,callback:v=>v+"%"},grid:{color:GRID},title:{display:true,text:"近3期期間觀看數百分位",color:TICK}}}}});
    const sleep=pts.filter(p=>p.x>=80&&p.y<=20).sort((a,b)=>(a.fps-b.fps)||(b.s-a.s)).slice(0,8);
    const dark=pts.filter(p=>p.x<=50&&p.y>=90).sort((a,b)=>b.rpv-a.rpv).slice(0,8);
    const over=pts.filter(p=>isFinite(p.shareRatio)).sort((a,b)=>b.shareRatio-a.shareRatio).slice(0,8);
    const zero=rows.filter(c=>c.rpv===0&&pctRank(subs,c.s)>=80).sort((a,b)=>b.s-a.s).slice(0,8);
    const gap=rows.filter(c=>c.rpv===0&&(!c.v||c.v===0)).sort((a,b)=>b.s-a.s).slice(0,8);
    const dormant=rows.filter(c=>c.rpv===0&&c.v>0).sort((a,b)=>b.s-a.s).slice(0,8);
    const block=(title,arr,extra)=>`<div class="faq-card"><h4>${title}</h4><table class="table"><thead><tr><th>頻道</th><th>訂閱</th><th>近3期期間觀看</th><th>${extra}</th></tr></thead><tbody>${arr.map(c=>`<tr><td>${avatar(c)}${escHtml(c.n||c.id)}</td><td>${fmt(c.s)}</td><td>${fmt(c.rpv)}</td><td>${extra==="注意力倍數"?c.shareRatio.toFixed(1)+"x":c.fps.toFixed(2)}</td></tr>`).join("")||'<tr><td colspan="4">目前沒有符合條件的頻道。</td></tr>'}</tbody></table></div>`;
    const zeroBlock=(title,arr)=>`<div class="faq-card"><h4>${title}</h4><table class="table"><thead><tr><th>頻道</th><th>訂閱</th><th>累計觀看</th><th>近3期期間觀看</th></tr></thead><tbody>${arr.map(c=>`<tr><td>${avatar(c)}${escHtml(c.n||c.id)}</td><td>${fmt(c.s)}</td><td>${fmt(c.v)}</td><td>${fmt(c.rpv)}</td></tr>`).join("")||'<tr><td colspan="4">目前沒有符合條件的頻道。</td></tr>'}</tbody></table></div>`;
    document.getElementById("concOutlier").innerHTML=`<div class="grid">${block("沉睡大台：訂閱高、近3期期間觀看低",sleep,"近3期/訂閱")}${block("流量黑馬：訂閱低、近3期期間觀看高",dark,"近3期/訂閱")}${block("注意力超配：期間觀看份額高於訂閱份額",over,"注意力倍數")}${zeroBlock("零期間觀看檢查：訂閱高但近3期為 0",zero)}${zeroBlock("資料疑似缺口：累計觀看也為 0",gap)}${zeroBlock("停更候選：累計觀看存在但近3期為 0",dormant)}</div><div class="footer">特異值只納入 Activity=Active 的頻道；主榜使用近 3 期期間觀看合計，且排除 0。零值另列為檢查清單，用來區分資料缺口與可能停更。</div>`;
  }
...
function buildReport(){
  ...
  function render(){const rows=ACT.filter(d=>!d.partial),y=repYear.value,cur=y==="latest"?rows[rows.length-1]:rows.filter(d=>d.quarter.startsWith(y)).slice(-1)[0],coh=COH.series.filter(d=>d.quarter.startsWith(cur.quarter.slice(0,4))),peak=COH.series.reduce((a,b)=>b.debuts>a.debuts?b:a),nat=Object.entries(COH.current.nationality).slice(0,3).map(([k,v])=>`${k} ${fmt(v)}`).join("、"),sh=cur.topvid_buckets||{},sht=Object.values(sh).reduce((a,x)=>a+x,0)||1,peakH=range(24).reduce((a,b)=>STM.yt.industry.hours[b]>STM.yt.industry.hours[a]?b:a,0);
    const dual=Object.keys(XPLAT.channels||{}).length, topDual=Object.values(XPLAT.channels||{}).filter(x=>x.yt_pct>=90&&x.tw_pct>=90).length, topShare=(cur.yt_top10_share*100).toFixed(1), viewShare=cur.yt_view_top10_share==null?"—":(cur.yt_view_top10_share*100).toFixed(1);
    const pr=(arr,v)=>{if(v==null||!arr.length)return null;let lo=0,hi=arr.length;while(lo<hi){const m=(lo+hi)>>1;if(arr[m]<v)lo=m+1;else hi=m;}return +(lo/arr.length*100).toFixed(1);};
    const concRows=(CONC.year||[]), conc=concRows.filter(r=>y==="latest"||String(r.label).startsWith(y)).slice(-1)[0]||concRows[concRows.length-1]||{channels:[]}, ci=Math.max(0,concRows.indexOf(conc)), win=concRows.slice(Math.max(0,ci-2),ci+1), flowById={};
    win.forEach(p=>(p.channels||[]).forEach(c=>{if(c.id&&c.pv!=null&&isFinite(c.pv))flowById[c.id]=(flowById[c.id]||0)+Math.max(0,c.pv);}));
    const activeRows=(conc.channels||[]).filter(c=>c.id&&c.ac==="Active"&&c.s>0).map(c=>Object.assign({},c,{rpv:flowById[c.id]||0})), subs=activeRows.map(c=>c.s).sort((a,b)=>a-b), highZero=activeRows.filter(c=>c.rpv===0&&pr(subs,c.s)>=80).length, gapZero=activeRows.filter(c=>c.rpv===0&&(!c.v||c.v===0)).length, dormantZero=activeRows.filter(c=>c.rpv===0&&c.v>0).length;
    const story=[["追蹤台V",fmt(cur.tracked_channels),`近期活躍 ${fmt(cur.recently_active_any)}，活躍率 ${cur.activation_rate}%`],["雙棲頻道",fmt(dual),`${fmt(topDual)} 個同時在 YT/Twitch 前 10%`],["集中度",topShare+"%",`前 10 大訂閱；累計觀看前 10 大 ${viewShare}%`],["近3期零值",fmt(highZero),`高訂閱 Active 頻道需分流檢查`],["開台尖峰",peakH+" 時",`這是開台時間，不是同接尖峰`]];
    const chapters=[["第一章","台V不是小圈圈，是一個可量測的長尾市場",fmt(cur.tracked_channels),`目前追蹤 ${fmt(cur.tracked_channels)} 個頻道，近期活躍 ${fmt(cur.recently_active_any)}。直播開場可以先問：這裡面誰真的還在動？`],["第二章","平台分工正在成形",fmt(dual),`雙棲頻道 ${fmt(dual)} 個，其中 ${fmt(topDual)} 個同時站進 YouTube/Twitch 前 10%。這適合接到雙棲象限圖。`],["第三章","集中度要拆成訂閱、累計觀看與期間觀看",topShare+"%",`前 10 大吃掉 ${topShare}% 訂閱、${viewShare}% 累計觀看；期間觀看改用近 3 期觀察，0 值另列檢查清單，避免把停更或資料缺口誤讀成沉睡大台。`],["第四章","作息資料能講開台生態，不能冒充同接",peakH+" 時",`YouTube 開台尖峰在台灣時間 ${peakH} 時。這是開台時間，不是觀看尖峰，避免把估算指標講成真同接。`]];
    const txt=[["產業規模",`追蹤頻道 <b>${fmt(cur.tracked_channels)}</b>，近期活躍 <b>${fmt(cur.recently_active_any)}</b>（活躍率 ${cur.activation_rate}%）。YouTube 訂閱中位數 ${fmt(cur.yt_subs_median)}，Twitch 近期活躍 ${fmt(cur.recently_active_twitch)}。`],["進出場動態",`出道高峰在 ${peak.quarter}，單季 ${peak.debuts} 人；${cur.quarter.slice(0,4)} 年出道 ${fmt(coh.reduce((a,x)=>a+x.debuts,0))}、畢業 ${fmt(coh.reduce((a,x)=>a+x.graduations,0))}。`],["內容趨勢",`熱門影片以 <b>${maxB(cur.topvid_buckets)}</b> 為主，短影音佔 ${((sh.shorts||0)/sht*100).toFixed(1)}%；直播內容主幹為 ${maxB(cur.yt_live_buckets)}。`],["平台版圖",`近期活躍中 YouTube ${fmt(cur.recently_active_yt)}、Twitch ${fmt(cur.recently_active_twitch)}；雙棲頻道 ${fmt(dual)}，其中 ${fmt(topDual)} 個同時位在 YouTube/Twitch 平台內前 10%。`],["集中度",`前 10 大佔全體訂閱 ${topShare}%、累計觀看 ${viewShare}%；訂閱中位數 ${fmt(cur.yt_subs_median)}。期間觀看數另以近 3 期合計看流量分配，主榜排除 0；高訂閱但近 3 期期間觀看為 0 的 Active 頻道有 ${fmt(highZero)} 個，其中累計觀看也為 0 的資料疑似缺口 ${fmt(gapZero)} 個，累計觀看存在但近 3 期為 0 的停更候選 ${fmt(dormantZero)} 個。`],["世代組成",`國籍前三為 ${nat}；團體 : 個人 = ${fmt(COH.current.group_split.group)} : ${fmt(COH.current.group_split.indie)}。`],["生命週期",`典型台 V 出道 12 個月生命週期中位為 ${GROW.lifecycle.median[12]} 倍初始訂閱，前段 p75 為 ${GROW.lifecycle.p75[12]} 倍。`],["開台生態",`YouTube 開台尖峰落在台灣時間 ${peakH} 時；作息資料已補完 7/7，但這是開台時間，不是觀看尖峰。`]];
